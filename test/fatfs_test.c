...
)
{
    ...
    for (cc = 1; cc <= ncyc; cc++) {
        ...
        printf(" disk_ioctl(%u, GET_SECTOR_COUNT, 0x%08X)", pdrv, &sz_drv);
        sz_drv = 0;
        ...
        printf(" disk_ioctl(%u, GET_SECTOR_SIZE, 0x%X)", pdrv, &sz_sect);
        sz_sect = 0;
        ...
        printf(" disk_ioctl(%u, GET_BLOCK_SIZE, 0x%X)", pdrv, &sz_eblk);
        sz_eblk = 0;
        ...
        if (ns > 4) ns = 4;
        for (n = 0, pn(pns); n < (sz_sect * ns); n++) pbuff[n] = (BYTE)pn(0);
        printf(" disk_write(%u, 0x%X, %lu, %u)", pdrv, pbuff, lba, ns);
        ...
        for (n = 0, pn(pns); n < (sz_sect * ns) && pbuff[n] == (BYTE)pn(0); n++) ;
        if (n == (sz_sect * ns)) {
            printf(" Data matched.\n");
        } else {
            printf("Failed: Read data differs from the data written.\n");
        ...
        printf(" disk_write(%u, 0x%X, %lu, 1)", pdrv, (pbuff+3), lba);
        ...
        printf(" disk_read(%u, 0x%X, %lu, 1)", pdrv, (pbuff+5), lba);
        ...
        if (sz_drv >= 128 + 0x80000000 / (sz_sect / 2)) {
            ...
            for (n = 0, pn(pns); n < (sz_sect * 2); n++) pbuff[n] = (BYTE)pn(0);
            printf(" disk_write(%u, 0x%X, %lu, 1)", pdrv, pbuff, lba);
            dr = disk_write(pdrv, pbuff, lba, 1);
            if (dr == RES_OK) {
                printf(" - ok.\n");
            ...
            printf(" disk_write(%u, 0x%X, %lu, 1)", pdrv, (pbuff+sz_sect), lba2);
            ...
            printf(" disk_read(%u, 0x%X, %lu, 1)", pdrv, (pbuff+sz_sect), lba2);
            ...
            for (n = 0, pn(pns); pbuff[n] == (BYTE)pn(0) && n < (sz_sect * 2); n++) ;
            if (n == (sz_sect * 2)) {
                printf(" Data matched.\n");
            } else {
                printf("Failed: Read data differs from the data written.\n");
